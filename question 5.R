...
write.csv(MLP_relation,'MLP_relation.csv',row.names = F)
...
MLP_relation %>% filter(writer==top_writer$writer[7]) %>% 
  ...
  ggraph(layout = "fr") +
  geom_edge_link(aes(edge_alpha = relation, edge_width = relation), edge_colour = "darkred") +
  geom_node_point(size = 5) +
  geom_node_text(aes(label = name), repel = TRUE,
                 point.padding = unit(0.2, "lines")) +
  theme_void()+labs(title=
                      paste0("Mane 6 relationship web of ",top_writer$writer[7])
                    )
