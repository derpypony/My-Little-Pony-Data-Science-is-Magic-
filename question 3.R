...
my_sentiment <- rbind(get_sentiments('b'), 
                      data.frame(word=c('no','not',"don't","haven't","won't", 
                                 ...
                                 sentiment='negative'))
# Please note that there are dulpicate in my_sentiment, remove duplicate
# Then we count how many positive/negative words appear in each article on average
my_sentiment <- my_sentiment[!duplicated(my_sentiment$word),]
for(i in 1:26)
{
  writer_name <- writer_score$writer[i]
  writer_content <- pony_synopsis %>% filter(str_detect(writer,writer_name))
  Total_number <- length(writer_content$writer)
  tidy_content <- writer_content %>% 
    unnest_tokens(word,content) %>% inner_join(my_sentiment)
...
# we are gonna need a data set for comparison purpose, we call it
# question6_data.csv
